<template>
  <div v-if="rating > 0" class="nugget-stars">
    <div>
      <component
        v-for="n of this.totalStars"
        :key="n"
        :is="currentComponent(n)"
        :fillColor="colorType(n)"
        :baseColor="baseColor"
        :rating="rating"
        :iconDimensions="iconDimensions"
      ></component>
    </div>
    <div v-if="totalReviews" class="nugget-reviews">{{ "(" + totalReviews + ") reviews" }}</div>
  </div>
  <div v-else id="nugget-stars-no-rating">{{ noRatingMsg }}</div>
</template>
